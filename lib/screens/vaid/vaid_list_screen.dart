...
import 'package:jivanand/screens/vaid/componet/vaid_list_widget.dart';
...
class VaidListScreen extends StatefulWidget {


  const VaidListScreen({super.key,});

  @override
  _CatListScreenState createState() => _CatListScreenState();
}

class _CatListScreenState extends State<VaidListScreen> with  SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    setStatusColor();
    init();
    LiveStream().on(LIVESTREAM_VAID_SCREEN, (p0) {
      init();
      setState(() {});
    });
  }
  ...
  List<VaidListData> serviceList = [];
  Timer? _debounce;

  void _getProduct() async {
    appStore.setLoading(true);
    await getVaidListWithPagination(page,
  ...
  void dispose() {
    ...
    _debounce?.cancel();
    super.dispose();
    LiveStream().dispose(LIVESTREAM_VAID_SCREEN);
  }
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      actions: [
          ...
          child: Text('Add Vaidya', style: boldTextStyle(color: white)),
          onPressed: () {
            AddVaidscreen().launch(context, pageRouteAnimation: PageRouteAnimation.Fade);
          },
        ).paddingRight(10)
      ],
      child: Column(
              ...
              if(serviceList.isNotEmpty)
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    VaidListData model=serviceList[index];
                    return VaidListWidget(model: model,
                      onDelete: () async {
                      appStore.setLoading(true);
                      await deleteVaid(id: model.id.validate().toString(),).then((value) async {
