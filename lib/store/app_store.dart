...
abstract class _AppStore with Store {
  ...
  @observable
  String empCode = getStringAsync(EMP_CODE);

  @observable
  String empPermission = getStringAsync(EMP_PERMISSION);

  @action
  Future<void> setEmpCode(String val) async {
    empCode = val;
    await setValue(EMP_CODE, val);
  }

  @action
  Future<void> setEmpPermission(String val) async {
    empPermission = val;
    await setValue(EMP_PERMISSION, val);
  }

  @observable
  int cartCount = getIntAsync(CART_COUNT);

  @action
  Future<void> setCartCount(int count, {bool isInitializing = false}) async {
    cartCount = count;
    if (!isInitializing) await setValue(CART_COUNT, count);
  }
