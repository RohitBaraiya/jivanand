...
import 'package:jivanand/component/app_common_dialog.dart';
import 'package:jivanand/component/html_widget.dart';
import 'package:jivanand/component/location_service_dialog.dart';
import 'package:jivanand/component/new_update_dialog.dart';
...
double homeSubIconSize = 30;
int homeSubTitleName = 13;

Color get bgColor {
  return appColorBg;
}
Color get cardColor {
  return newCardColor1;
}
...
Future setStatusColor() async {
  ...
  /*if (appStore.isDarkMode) {
    setStatusBarColor(darkCard, statusBarBrightness: Brightness.dark, statusBarIconBrightness: Brightness.light);
  } else {
    setStatusBarColor(lightCard, statusBarBrightness: Brightness.dark, statusBarIconBrightness: Brightness.dark);
  }*/
  setStatusBarColor(newCardColor1, statusBarBrightness: Brightness.dark, statusBarIconBrightness: Brightness.dark);
}
...
bool isTablet(BuildContext context) {
  ...
  // Assuming a width greater than 600 is considered a tablet
  return width >= 600;
}



Future<bool> addToCart({required CardModel model}) async {
  bool exists = await DBHelper.instance.doesRecordExist(model.pId.validate().toInt());
  if(!exists){
    int id = await DBHelper.instance.insert(model);
    LiveStream().emit(LIVESTREAM_OFFER_PRODUCT_DASHBOARD);
    if (id > 0) {
      // print('Inserted card with ID: $id');
      int count = await DBHelper.instance.getRecordCount();
      appStore.setCartCount(count);
      return true;
    } else {
      // print('Insert failed');
      return false;
...
Future<bool> removeToCart({required CardModel model}) async {
  int id = await DBHelper.instance.delete(model.id.validate());
  if (id > 0) {
   // LiveStream().emit(LIVESTREAM_OFFER_PRODUCT_DASHBOARD);
    // print('Inserted card with ID: $id');
    int count = await DBHelper.instance.getRecordCount();
...
InputDecoration inputDecoration(BuildContext context, {Widget? prefixIcon, String? labelText, double? borderRadius}) {
  return InputDecoration(
    ...
    labelText: labelText,
    labelStyle: secondaryTextStyle(size: 14,color: textColor.withOpacity(0.8),),
...
InputDecoration inputDecoration2(BuildContext context, {Widget? prefixIcon, String? labelText, double? borderRadius}) {
  return InputDecoration(
    contentPadding: const EdgeInsets.only(left: 12, bottom: 10, top: 10, right: 10),
    labelText: labelText,
    labelStyle: secondaryTextStyle(size: 14,color: textColor.withOpacity(0.8),),
...
InputDecoration otp_inputDecoration(BuildContext context, {Widget? prefixIcon, Widget? prefix, String? hint, Color? fillColor, String? counterText, double? borderRadius}) {
  return InputDecoration(
    contentPadding: const EdgeInsets.only(left: 12, bottom: 10, top: 10, right: 10),
    labelText: hint,
